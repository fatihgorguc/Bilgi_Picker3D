...
namespace Controllers.Player
{
    public class PlayerPhysicsController : MonoBehaviour
    {
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            if (other.CompareTag("MiniGameStartArea"))
            {
                //new
                CoreGameSignals.Instance.onMinigameStartAreaEntered?.Invoke();
                //Write Mini Game Conditions
            }

            if (other.CompareTag("MiniGamePoolArea"))
            {
                //new
                manager.ForceCommand.Execute();
                CoreGameSignals.Instance.onMinigamePoolAreaEntered?.Invoke();
                DOVirtual.DelayedCall(1.3f, () =>
                {
                    ...
                    if (result)
                    {
                        CoreGameSignals.Instance.onMinigamePoolAreaSuccessful?.Invoke(manager.StageValue);
                    }
                });
            }
        }
